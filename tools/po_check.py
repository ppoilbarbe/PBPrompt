#!/usr/bin/env python3
"""
PO file inspection tool for PBPrompt.
...
try:
    ...
    sys.exit("babel is not installed.  Run: conda install -n pbprompt babel")
...
def print_stats(langs):
    header = f"{'lang':<8} {'total':>6} {'transl':>8} {'missing':>8} {'fuzzy':>6}"
    print(header)
    print("-" * len(header))
    for lang in langs:
        s = stats_for(lang)
        ...
        print(
            f"{s['lang']:<8} {s['total']:>6} {s['translated']:>8}"
            f" {miss:>8} {s['fuzzy']:>6}"
        )
